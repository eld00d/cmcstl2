...
namespace associated_type_test {
	...
	CONCEPT_ASSERT(ranges::Same<int&, ns::reference_t<C>>());
	CONCEPT_ASSERT(ranges::Same<int&, ns::reference_t<D>>());
	...
	CONCEPT_ASSERT(ranges::Same<int&&, ns::rvalue_reference_t<C>>());
	CONCEPT_ASSERT(ranges::Same<int&&, ns::rvalue_reference_t<D>>());
...
#if VALIDATE_STL2
	CONCEPT_ASSERT(ranges::Same<int, ns::value_type_t<C>>());
	CONCEPT_ASSERT(ranges::Same<double, ns::value_type_t<D>>());
#endif
	...
	CONCEPT_ASSERT(ranges::Same<ns::iterator_category_t<D>, ns::forward_iterator_tag>());
#if VALIDATE_STL2
...
namespace indirectly_callable_test {
	CONCEPT_ASSERT(ranges::IndirectCallable<std::plus<int>, int*, int*>());
}
